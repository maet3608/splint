"""
.. module:: test_docstrlint
   :synopsis: Unit tests for docstrlint module
"""
...
def test_constructor_ModDef():
    ...
    module = ast.parse(code)
    definition = ad.ModDef(module, 'foo.py')
    assert definition.name == 'foo.py'
...
def test_parameters_FnDef():
    ...
    assert ad.FnDef.parameters(fn_node) == []

    fn_node = function_node("def add(a,b): pass")
    assert ad.FnDef.parameters(fn_node) == ['a', 'b']

    fn_node = function_node("def disp(*args): pass")
    assert ad.FnDef.parameters(fn_node) == ['args']

    fn_node = function_node("def disp(**kwargs): pass")
    assert ad.FnDef.parameters(fn_node) == ['kwargs']

    fn_node = function_node("def disp(p1, *args, **kwargs): pass")
    assert ad.FnDef.parameters(fn_node) == ['p1', 'args', 'kwargs']


def test_has_term_FnDef():
    fn_node = function_node("def add(a,b): return a+b")
    assert ad.FnDef._has_term(fn_node, ast.Return) == True

    fn_node = function_node("def fail(): raise ValueError('')")
    assert ad.FnDef._has_term(fn_node, ast.Raise) == True

    fn_node = function_node("def write(a): print a")
    assert ad.FnDef._has_term(fn_node, ast.Return) == False


def test_decorators_FnDef():
    code = read('class_with_staticmethod.py')
    fn_node = function_node(code)
    assert ad.FnDef._decorators(fn_node) == {'staticmethod'}


def test_ignore_first_param_FnDef():
    fn_def = ad.FnDef(function_node("def add(a,b): return a+b"), False)
    assert fn_def._ignore_first_param() == False
    code = read('class_with_staticmethod.py')
    fn_def = ad.FnDef(function_node(code), True)
    assert fn_def._ignore_first_param() == False
...
def test_check_docstring_FnDef():
    fn_def = ad.FnDef(function_node("def add(a,b): return a+b"), False, False)
    assert fn_def._check_docstring() == False
    fn_def = ad.FnDef(read_function('add_with_docstr.py'), False, False)
...
def test_check_params_additional_FnDef():
    ...
    fn_def = ad.FnDef(function_node(code), False, False)
...
def test_construction_DocStr():
    docstr = ad.DocStr(read_function('add_with_docstr.py'))
...
def test_find_DocStr():
    ...
    assert ad.DocStr._find(r':param\s(.+):(.+)', code) == [
        ['int a', 'First number.'], ['int b', 'Second number.']]


def test_param_annotations_DocStr():
    code = read('add_with_docstr.py')
    assert ad.DocStr.param_annotations(code) == [['int', 'a', 'First number.'],
                                                 ['int', 'b', 'Second number.']]


def test_return_annotations_DocStr():
    code = read('add_with_docstr.py')
    assert ad.DocStr.return_annotations(code) == \
           [['return', 'Sum of the two numbers.'], ['rtype', 'int']]


def test_ignore():
    node = function_node("def foo(): pass")
    assert not ad.ignore(node)
    node = function_node("def __init__(self): pass")
    assert not ad.ignore(node)
    node = function_node("def _foo(): pass")
    assert ad.ignore(node)


def test_walk_ast(clear_report):
    node = class_node(read('class_with_method.py'))
    ar.Report.new_file('class_with_method.py')
    ad.walk_ast(node.body, False)
    assert ar.Report._rfiles
    assert len(ar.Report._rfile.definitions) == 1
    node = class_node(read('class_with_private_method.py'))
    ar.Report.new_file('class_with_private_method.py')
    ad.walk_ast(node.body, False)
    assert ar.Report._rfiles
    assert len(ar.Report._rfile.definitions) == 0


def test_lint(clear_report):
    ad.lint('testdata/class_with_method.py')
    assert ar.Report._rfiles
    assert len(ar.Report._rfile.definitions) == 3


def test_python_files():
    files = list(ad.python_files('tests'))
    assert len(files) == 4
